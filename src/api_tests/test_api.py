...
def test_math_api_happy_path():
    r = requests.post("%s/demo/math" % URL, json={'item_a': '9', 'item_b': '7'})
    assert "16" in r.text
...
def test_register_api():
    __reset_database()
    r = requests.post("%s/demo/register" % URL, json={'username': 'alice', 'password': 'B65S3xNW8vXQHyjYnD72L3mejc'})
    assert "successfully registered: true" in r.text

# register with an empty password
def test_register_api_empty_password():
    __reset_database()
    r = requests.post("%s/demo/register" % URL, json={'username': 'alice', 'password': ''})
    assert "no password provided" in r.text

# register with an empty username
def test_register_api_empty_username():
    __reset_database()
    r = requests.post("%s/demo/register" % URL, json={'username': '', 'password': 'B65S3xNW8vXQHyjYnD72L3mejc'})
...
def test_login_api():
    __reset_database()
    requests.post("%s/demo/register" % URL, json={'username': 'alice', 'password': 'B65S3xNW8vXQHyjYnD72L3mejc'})
    r = requests.post("%s/demo/login" % URL, json={'username': 'alice', 'password': 'B65S3xNW8vXQHyjYnD72L3mejc'})
...
def test_login_api_empty_username():
    __reset_database()
    requests.post("%s/demo/register" % URL, json={'username': 'alice', 'password': 'B65S3xNW8vXQHyjYnD72L3mejc'})
    r = requests.post("%s/demo/login" % URL, json={'username': '', 'password': 'B65S3xNW8vXQHyjYnD72L3mejc'})
    assert "no username provided" in r.text
...
def test_login_api_empty_password():
    __reset_database()
    requests.post("%s/demo/register" % URL, json={'username': 'alice', 'password': 'B65S3xNW8vXQHyjYnD72L3mejc'})
    r = requests.post("%s/demo/login" % URL, json={'username': 'alice', 'password': ''})
    assert "no password provided" in r.text
...
def test_register_book():
    __reset_database()
    r = requests.post("%s/demo/registerbook" % URL, json={'book': 'alice in wonderland'})
...
def test_register_book_already_registered():
    __reset_database()
    requests.post("%s/demo/registerbook" % URL, json={'book': 'alice in wonderland'})
    r = requests.post("%s/demo/registerbook" % URL, json={'book': 'alice in wonderland'})
...
def test_register_borrower():
    __reset_database()
    r = requests.post("%s/demo/registerborrower" % URL, json={'borrower': 'alice'})
    assert "SUCCESS" in r.text
...
def test_register_borrower_already_registered():
    __reset_database()
    requests.post("%s/demo/registerborrower" % URL, json={'borrower': 'alice'})
    r = requests.post("%s/demo/registerborrower" % URL, json={'borrower': 'alice'})
...
def test_create_book_loan():
    ...
    requests.post("%s/demo/registerborrower" % URL, json={'borrower': 'alice'})
    requests.post("%s/demo/registerbook" % URL, json={'book': 'alice in wonderland'})
    r = requests.post("%s/demo/lend" % URL, json={'borrower': 'alice', 'book': 'alice in wonderland'})
    assert "SUCCESS" in r.text
...
def test_create_book_loan_already_lent():
    ...
    requests.post("%s/demo/registerborrower" % URL, json={'borrower': 'alice'})
    requests.post("%s/demo/registerbook" % URL, json={'book': 'alice in wonderland'})
    requests.post("%s/demo/lend" % URL, json={'borrower': 'alice', 'book': 'alice in wonderland'})
    r = requests.post("%s/demo/lend" % URL, json={'borrower': 'alice', 'book': 'alice in wonderland'})
